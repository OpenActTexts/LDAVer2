...
*Chapter Preview*. The appendix gives an overview of concepts and methods related to statistical inference on the population of interest, using a random sample of observations from the population. In the appendix, Section \@ref(S:Sec171) introduces the basic concepts related to the population and the sample used for making the inference. Section \@ref(S:Sec172) presents the commonly used methods for point estimation of population characteristics. Section \@ref(S:Sec173) demonstrates interval estimation that takes into consideration the uncertainty in the estimation, due to use of a random sample from the population. Section \@ref(S:Sec174) introduces the concept of hypothesis testing for the purpose of variable and model selection.

## Basic Concepts {#S:Sec171}
...
**Example -- Wisconsin Property Fund.** Assume there are 1,377 *individual* claims from the 2010 experience. Summary statistics are in Table \@ref(tab:Tab171) and a visual summary is in Figure \@ref(fig:Fig171).

```{r Tab171, echo=FALSE}
library(doBy)
ClaimLev <- read.csv("Data/CLAIMLEVEL.csv", header=TRUE)
ClaimLevBC10<-subset(ClaimLev,Year==2010); 
ClaimLevBC10$LogClaim <- log(ClaimLevBC10$Claim)

t1<- as.numeric(doBy::summaryBy(Claim ~ 1, data = ClaimLevBC10, 
     FUN = function(x) { c(ma=min(x), p25 = quantile(x,prob = 0.25),
                           m1=median(x),m=mean(x),p75 = quantile(x,prob = 0.75),
                           mb=max(x),sd=sd(x) ) } ) )
t1 <- roundCom(t1, digits = 0)
t2<- as.numeric(doBy::summaryBy(LogClaim  ~ 1, data = ClaimLevBC10, 
     FUN = function(x) { c(ma=min(x), p25 = quantile(x,prob = 0.25),
                           m1=median(x),m=mean(x),p75 = quantile(x,prob = 0.75),
                           mb=max(x),sd=sd(x) ) } ) )
t2 <- round(t2, digits = 3)
tableout <- rbind(t1,t2) 
colnames(tableout) <- c("Minimum", "First Quartile",
                        "Median","Mean", "Third Quartile",
                        "Maximum", "Standard Deviation")
rownames(tableout) <- c("Claim","Logarithmic Claims")
TableGen1(TableData=tableout,
         TextTitle= 'Wisconsin Property Fund Summary Statistics',
         Align='c', Digits=3, ColumnSpec=1:7,
         ColWidth = ColWidth7)

```

(ref:Fig171) **Distribution of Claims for Wisconsin Property Fund**

```{r Fig171, fig.cap='(ref:Fig171)', out.width='80%', fig.asp=.75, echo=FALSE}
ClaimLev <- read.csv("Data/CLAIMLEVEL.csv", header=TRUE)
ClaimLevBC10<-subset(ClaimLev,Year==2010); 
...
`r HideExample('SI', 'Show R Code Empirical Example Using the Wisconsin Property Fund')`


```{r ref.label = 'Tab171', echo=HtmlEval, eval = FALSE}
```

```{r ref.label = 'Fig171', echo=HtmlEval, eval = FALSE}
```
...
### Sampling Distribution {#S:Sec1712}
...
### Central Limit Theorem {#S:Sec1713}
...
$$
\sqrt{n}(\bar{X}-\mu)\rightarrow^{d}N(0,\sigma).
$$  
...
## Point Estimation and Properties {#S:Sec172}
...
### Method of Moments Estimation  {#S:Sec1721}
...
### Maximum Likelihood Estimation {#S:Sec1722}
...
Hence, the sample mean $\bar{X}$ and $\hat{\sigma}^2$ are both the *mme* and `r Gloss('MLE')` for the mean $\mu$ and variance $\sigma^2$, under a normal population distribution $F(\cdot)$. More details regarding the properties of the likelihood function are given in Appendix Section \@ref(S:Sec191).

## Interval Estimation {#S:Sec173}
...
### Exact Distribution for Normal Sample Mean {#S:Sec1731}
...
### Student-*t* test based on *mle*  {#S:Sec1742}

Based on the results from Section \@ref(S:Sec1731), we can define a Student-$t$ test for testing $H_0:\theta=\theta_0$. In particular, we define the test statistic as
$$
...
**Example -- Wisconsin Property Fund.** For numerical stability and extensions to regression applications, statistical packages often work with transformed versions of parameters. Table \@ref(tab:Tab172) provides estimates based on the **R** package **VGAM** (the function). More details on the *mle* of other distribution families are given in Appendix Chapter \@ref(CAppC).



```{r Tab172, echo=FALSE}

  Row1Gamma <- c(10.190, 0.050, 203.831)
  Row2Gamma  <- c(-1.236, 0.030, -41.180)
  Row1Lognormal <- c(7.804, 0.045, 172.089)
  Row2Lognormal <- c(0.520, 0.019, 27.303)
  Row1Pareto  <- c(7.733,  0.093, 82.853)
  Row2Pareto <- c(-0.001, 0.054, -0.016)
  Row1GB2 <- c(2.831, 1.000,  2.832)
  Row2GB2 <- c(1.203, 0.292 , 4.120)
  Row3GB2 <- c(6.329, 0.390 , 16.220)
  Row4GB2 <- c(1.295, 0.219 , 5.910)

tableout <- rbind(Row1Gamma,Row2Gamma,
                  Row1Lognormal,Row2Lognormal,
                  Row1Pareto,Row2Pareto,
                  Row1GB2, Row2GB2,
                  Row3GB2, Row4GB2) 
colnames(tableout) <- c("Parameter Estimate", "Standard Error",
                        "$t$-stat")
rownames(tableout) <- c("Gamma","",
                        "Lognormal", "",
                        "Pareto", "", 
                        "GB2", "", "", "")
TableGen1(TableData=tableout,
         TextTitle= 'Wisconsin Property Fund Parameter Estimates',
         Align='r', Digits=3, ColumnSpec=1:2,
         ColWidth = ColWidth4)
...
### Likelihood Ratio Test {#S:Sec1743}
...
### Information Criteria {#S:Sec1744}
...
(ref:Fig172) **Fitted Claims Distribution**

```{r Fig172, fig.cap='(ref:Fig172)', out.width='80%', fig.asp=.75, echo=FALSE}
ClaimLev <- read.csv("Data/CLAIMLEVEL.csv", header=TRUE); 
...
```{r ref.label = 'Fig172', echo=HtmlEval, eval = FALSE}
```
