...
The objective of this chapter is to build a probability model to describe the aggregate claims by an insurance system occurring in a fixed time period. The insurance system could be a single policy, a group insurance contract, a business line, or an entire book of an insurer's business. In this chapter, `r Gloss('aggregate claims')` refer to either the number or the amount of claims from a portfolio of insurance contracts. However, the modeling framework can be readily applied in a general setup.
...
The rest of the chapter is structured as follows: Section \@ref(S:Sec72) and Section \@ref(S:Sec73) provide details on the individual risk model and collective risk model respectively. Section \@ref(S:Sec74) presents methods for computing the distribution of aggregate claims. Section \@ref(S:Sec75) discusses the effect of coverage modifications on the aggregate losses. Technical materials are summarized in Section \@ref(S:Sec76).
...
### Moments and Distribution {#S:Sec721}
...
`r SolnBegin()` The aggregate claims are $S_{300} = X_1+\cdots+X_{300}$, where $X_1, \ldots, X_{300}$ are independent but not identically distributed. Policy claims amounts are uniformly distributed on $(0,M_i)$, so the mean claim amount is $M_i/2$ and the variance is $M_i^2/12$. Thus, for policy $i=1,\ldots,300$, we have
$$
{\small 
\begin{matrix}
...
`r SolnBegin()` Each policy claim amount $X_i$ is now deterministic and fixed at $M_i$ instead of a randomly distributed amount, so $\sigma_i^2 = \mathrm{Var~} (X_i) = 0$ and $\mu_i = M_i$. Again, the probability of a claim occurring for each policy is $q_i$. Under these circumstances, the expected aggregate loss is
$$
\begin{aligned}
...
`r SolnBegin()` For each policy, we can write the zero-modified Poisson claim frequency $N_i$ as $N_i = I_i \times B_i$, where
$$
...
### Aggregate Loss Distribution {#S:Sec722}
...
$$
\begin{aligned}
 F_{S_n}(s) &= {\rm Pr}({S_n}\le s) = \Pr \left( \frac{S_n - \mu_{S_n}}{\sigma_{S_n}} \le \frac{s-\mu_{S_n}}{\sigma_{S_n}} \right) \\
...
**Example 7.2.3. Actuarial Exam Question - Follow-Up.**
As in the Example 7.2.1 earlier, an insurance company sold 300 fire insurance policies, with claim amounts $X_i$ uniformly distributed between 0 and the policy maximum $M_i$. Using the normal approximation, calculate the probability that the aggregate claim amount $S_{300}$ exceeds $\$3,500$. 

`r HideExample('5.2.3','Show Example Solution')`

`r SolnBegin()` We have seen earlier that $\mathrm{E}(S_{300})=2,800$ and $\mathrm{Var}(S_{300}) = 600,467$. Then
$$
\begin{aligned}
{\rm Pr}(S_{300} > 3,500) &= 1 - {\rm Pr}(S_{300} \le 3,500) \\
...
For $k=1$:
$$
F_{S_1}(s) = {\rm Pr}(S_1\le s) = {\rm Pr}(X_1\le s) = F_{X_1}(s).
$$
For $k=2,\ldots,n$:
$$
\begin{aligned}
    F_{S_k}(s)&={\Pr}(X_1+\cdots+X_k\le s) ={\Pr}(S_{k-1}+X_k\le s) \\
    &={\rm E}_{X_k}\left[{\rm Pr}(S_{k-1}\le s-X_k|X_k)\right]= {\rm E}_{X_k}\left[F_{S_{k-1}}(s-X_k)\right] .
   \end{aligned}
$$
...
$$
F^{*n}_X(x)={\Pr}(X_1+\cdots+X_n\le x) ,
$$
...
**Example 7.2.4. Gamma Distribution.** Assume that $X_1,\ldots,X_n$ are independent random variables with $X_i \sim Gam(\alpha_i, \theta)$. The *mgf* of $X_i$ is $M_{X_i}(t) = (1 - \theta t)^{- \alpha_i}$. Thus, the *mgf* of the sum $S_n = X_1 + \cdots + X_n$ is
$$
\begin{aligned}
...
**Example 7.2.5. Negative Binomial Distribution.** Assume that $X_1,\ldots, X_n$ are independent random variables with $X_i \sim NB(\beta, r_i)$. The *pgf* of $X_i$ is $P_{X_i}(z) = \left[1-\beta(z-1) \right]^{-r_i}$. Thus, the *pgf* of the sum $S_n =X_1+\cdots+X_n$ is
$$
\begin{aligned}
...
### Moments and Distribution {#S:Sec731}
...
\end{aligned}
$$
Using the `r Gloss('law of iterated expectations')` from Appendix Section \@ref(S:Sec182), the mean of the aggregate loss is
$$
{\rm E}(S_N)={\rm E}_N[{\rm E}_S(S|N)] = {\rm E}_N(N\mu) = \mu ~{\rm E}(N).
$$
Using the `r Gloss('law of total variance')` from Appendix Section \@ref(S:Sec182), the variance of the aggregate loss is
$$
\begin{aligned}
...
`r HideExample('7.3.1','Show Example Solution')`
...
**Example 7.3.2. Actuarial Exam Question.**
...
`r HideExample('7.3.2','Show Example Solution')`

`r SolnBegin()`  We need to calculate the mean and standard deviation of the aggregate (sum) of cash prizes. The moments of the frequency distribution $N$ are
$$
\begin{aligned}
...
`r HideExample('7.3.3','Show Example Solution')`

`r SolnBegin()` By definition, we have
$$
\begin{aligned}
F_{S_N}\left(3 \right) &= {\rm Pr}\left(\sum_{i=1}^N X_i \le 3\right) = \sum_{n=0}^\infty {\rm Pr}\left(\sum_{i=1}^n X_i\le 3|N=n\right){\rm Pr}(N=n) \\
...
&= \frac{1}{4}\left[F^{\ast 1} (2) + F^{\ast 1}(1)\right] = \frac{1}{4}\left[{\rm Pr}(X\le 2) + {\rm Pr}(X \le 1) \right] \\
&= \frac{1}{4} \left(\frac{2}{4} + \frac{1}{4} \right) = \frac{3}{16}\\
F^{\ast 3}(3) &= \Pr(X_1+X_2 + X_3 \le 3) = \Pr(X_1=X_2=X_3=1) = \left(\frac{1}{4} \right)^3 .
\end{aligned}
$$
Notice that we did not need to recursively calculate $F^{\ast 3}(3)$ by recognizing that each $X \in \{1,2,3,4\}$, so the only way of obtaining $X_1+X_2+X_3 \le 3$ is to have $X_1=X_2=X_3=1$. Additionally, for $n \geq 4$, $F^{\ast n} (3)=0$ since it is impossible for the sum of 4 or more $X$'s to be less than 3. For $n=0$, $F^{\ast 0}(3) = 1$ since the sum of 0 $X$'s is 0, which is always less than 3. Laying out the probabilities systematically,  
...
With the parameter estimates in place, we are now in a position to calculate distribution of $S=X_1+X_2+\cdots+X_N$ using convolution method. Figure \@ref(fig:Fig71) summarizes the aggregate loss distribution. The following block of code demonstrates its calculation.
...
(ref:Fig71) **Aggregate Loss Distribution for Wisconsin Property Fund Building and Loss Claims**

```{r Fig71, fig.cap='(ref:Fig71)', out.width='80%', echo=FALSE}
...
`r HideExample('7.3.5','Show Example Solution')`

`r SolnBegin()`   To use the normal approximation, we must first find the mean and variance of the aggregate loss $S$
$$
\begin{aligned}
...
[Example 7.3.6]: ./ChapAggLossModels.html#Ex:7.3.6

**[Example 7.3.6]{#Ex:7.3.6}. Actuarial Exam Question.** For an individual over 65:  
...
`r HideExample('7.3.6','Show Example Solution')`

`r SolnBegin()`  We have claim frequency $N \sim Poi(\lambda = 25)$ and claim severity $X \sim U \left(5, 95 \right)$. To use the normal approximation, we need to find the mean and variance of the aggregate claims $S_N$. Note  
$$
\begin{matrix}
\begin{array}{lll}
...
### Stop-loss Insurance {#S:Sec732}

Recall the coverage modifications on the individual policy level in Section \@ref(S:Sec51). Insurance on the aggregate loss $S_N$, subject to a `r Gloss('deductible')` $d$, is called *net stop-loss insurance*. The expected value of the amount of the aggregate loss in excess of the deductible,
\begin{eqnarray*}
\mathrm{E}[(S_N-d)_+]
\end{eqnarray*}
...
`r HideExample('7.3.7','Show Example Solution')`

`r SolnBegin()`  The number of projects in a week requiring overtime work has distribution $N \sim Geo(\beta=2)$, while the number of overtime hours worked per project has distribution $X$ as described above. The aggregate number of overtime hours in a week is $S_N$ and we are therefore looking for
$$
...
**Example 7.3.8. Actuarial Exam  Question - Continued.**
...
$$
\begin{aligned}
	\mathrm{E~}(S_N-5)_+ &= \mathrm{E}(S_N) - 5 [1-\Pr(S_N \le 0) ]\\ %\Pr (S_N\geq 5) \\
...
$$
\begin{aligned}
	\mathrm{E~}(S_N-10)_+ &= \mathrm{E~}(S_N-5)_+ - 5 [1-\Pr(S_N \le 5)]\\ %\Pr (S_N\ge 10) \\
...
$$
\begin{aligned}
	\mathrm{E~}(S_N-15)_+ &= \mathrm{E~}(S_N-10)_+ - 5 [1-\Pr(S_N \le 10)] \\ %\Pr (S_N\ge 15)  \\
...
### Closed-form Distributions {#S:Sec733}
...
**Example 7.3.10. Exponential Severity.** Consider a collective risk model with an exponential severity and an arbitrary frequency distribution. Recall that if $X_i\sim Exp(\theta)$, then the sum of *iid* exponential random variables, $S_n=X_1+\cdots+X_n$, has a gamma distribution, i.e. $S_n\sim Gam(n,\theta)$. This has cdf:
\begin{eqnarray*}
...
## Computing the Aggregate Claims Distribution {#S:Sec74}
...
**Example 7.4.1. Actuarial Exam Question.**
...
`r SolnBegin()`  The severity distribution $X$ follows
$$
...
1. Individual Risk Model: $S_n=X_1+\cdots+X_n$    
    ...
    + Generate each individual loss realization $x_{ij}$ for $i=1,\ldots,n$. For example, this can be done using the inverse transformation method (Section \@ref(S:Sec812)).
...
$$
\hat{F}_S(s)=\frac{1}{R}\sum_{j=1}^{R}I(s_j\le s),
$$
...
[Example 7.4.3]: ./ChapAggLossModels.html#Ex:7.4.3

**[Example 7.4.3]{#Ex:7.4.3}.** Recall [Example 7.3.6](Ex:7.3.6) with an individual's claim frequency $N$ has a Poisson distribution with mean $\lambda=25$ and claim severity $X$ is uniformly distributed on the interval $(5,95)$. Using a simulated sample of 10,000 observations, estimate the mean and variance of the aggregate loss $S_N$. In addition, use the simulated sample to estimate the probability that aggregate claims for this individual will exceed 2,000 and compare with the normal approximation estimates from Example 7.3.6.
...
## Effects of Coverage Modifications {#S:Sec75}
...
### Impact of Exposure on Frequency {#S:Sec751}
...
S=X_1+\cdots+X_n ,
$$
where we assume $X_i$ are *iid* representing the number of claims from policy $i$. In this case, the exposure for the portfolio is $n$, using policy as exposure base. In Section \@ref(S:Sec104) we will introduce other exposure bases. The *pgf* of $S$ is
$$
\begin{aligned}
...
`r SolnBegin()`  Each of the 5 households has number of exposures $n_j$ (number of vehicles) and number of claims $S_j$, $j=1,...,5$. Note for each household, the number of claims $S_j \sim Poi (n_j \lambda)$. The likelihood function is  
$$
\begin{aligned}
...
### Impact of Deductibles on Claim Frequency {#S:Sec752}
...
`r SolnBegin()`  Because the loss frequency $N^L$ is Poisson, we can relate the means of the loss distribution $N^L$ and the first payment distribution $N^{P}_1$ (under deductible $d_1=30$) through $0.4 = \lambda v_1$, where
$$
\begin{aligned}
...
Next, we examine the more general case where $N^L$ is a zero-modified distribution. Recall that a zero-modified distribution can be defined in terms of an unmodified one (as was shown in Section \@ref(S:Sec351)). That is,
$$
\begin{aligned}
...
`r SolnBegin()`  Since $N^L$ follows a ZM-Poisson distribution with parameters $\lambda$ and $p_0^M$, we know that $N^P$ also follows a ZM-Poisson distribution, but with parameters $\lambda v$ and $p_0^M$, where
$$
...
### Impact of Policy Modifications on Aggregate Claims {#S:Sec753}
...
$N^P$ for the number of payments (as defined in the previous section \@ref(S:Sec752)). Also, define the amount of payment on a `r Gloss('per-loss basis')` as
\begin{eqnarray*}
    Y^{L}&=\left\{
      \begin{array}{ll}
        0 ~, & \text{if } ~X<\cfrac{d}{1+r} \\
        \alpha[(1+r)X-d]~, & \text{if } ~\cfrac{d}{1+r}\le X<\cfrac{u}{1+r} \\
...
\end{aligned}
$$
(Recall that when we introduced the per-loss and per-payment bases in Section \@ref(S:Sec51), we used another letter $Y$ to distinguish losses from insurance payments, or claims. At this point in our development, we use the letter $X$ to reduce notation complexity.)
...
**Example 7.5.5. Actuarial Exam Question.**
...
`r SolnBegin()`  The cost per loss with a 50\% increase in severity and a 100 deductible per claim is
\begin{eqnarray*}
X^L &=&
\left\{
\begin{array}{cc}
...
**Example 7.5.6. Follow-Up.** What is the variance of the total claim payment, $\mathrm{Var~}(S)$?  


`r HideExample('5.5.6','Show Example Solution')`

`r SolnBegin()`   On a per loss basis, we have
$$\begin{aligned}
...
Thus, the number of payments, $N^P$ has a negative binomial distribution (see negative binomial special case in Section \@ref(S:Sec752))
with mean
...
`r SolnBegin()` On a per loss basis, we have a 100 deductible. Thus, the expectation per loss is
$$
\begin{aligned}
...
## Further Resources and Contributors {#S:Sec76}
...
If you would like additional practice with `R` coding, please visit our companion [LDA Short Course](https://openacttexts.github.io/LDACourse1). In particular, see the [Aggregate Loss Models Chapter](https://openacttexts.github.io/LDACourse1/aggregate-loss-models.html).
...
$$
\begin{aligned}
\mathrm{E}(S_n) &=\sum_{i=1}^n ~ \mathrm{E}(X_i) = \sum_{i=1}^n ~ \mathrm{E}(I_i \times B_i) = \sum_{i=1}^n ~ \mathrm{E}(I_i) ~~ \mathrm{E}(B_i) ~~~~ \text{from independence of } I_i \text{'s and } B_i \text{'s} \\
&= \sum_{i=1}^n \Pr(I_i=1) ~ \mu_i ~~~~ \text{since } E(I_i) \text{ is the probability } I_i \text{ is 1} \\
&= \sum_{i=1}^n ~ q_i ~ \mu_i .
\end{aligned}
$$
...
$$
\begin{aligned}
\mathrm{Var}(S_n) &= \sum_{i=1}^n \mathrm{Var}(X_i) ~~~~ \text{from independence of } X_i \text{'s} \\
&= \sum_{i=1}^n ~ \left( ~ \mathrm{E}\left[ \mathrm{Var}(X_i | I_i) \right] + \mathrm{Var}\left[ \mathrm{E}(X_i|I_i) \right] ~ \right) ~~~~ \text{from conditional variance formulas}  \\
...
&= \prod_{i=1}^n ~ \mathrm{E}(z^{~X_i}) = \prod_{i=1}^n ~ \mathrm{E}(z^{~I_i \times B_i}) = \prod_{i=1}^n ~\mathrm{E} \left[ \mathrm{E}(z^{~I_i \times B_i} | I_i) \right] ~~~~ \text{from law of iterated expectations} \\
...
& = \prod_{i=1}^n ~ \mathrm{E} \left[ \mathrm{E} \left( e^{~t~(I_i \times B_i)} | I_i \right) \right] ~~~~ \text{from law of iterated expectations} \\
...
$$
\begin{aligned}
P_{X_i}(z) &= \mathrm{E~}(z^{X_i}) = \mathrm{E}\left[  \mathrm{E}\left( z^{X_i} | X_i \right) \right] ~~~~ \text{from law of iterated expectations} \\ 
&= \mathrm{E}\left( z^{X_i} | X_i=0 \right)~ \Pr(X_i=0) + \mathrm{E}\left( z^{X_i} | X_i>0 \right) ~ \Pr(X_i>0) \\
&= (1)~ p_{i0} + \mathrm{E}(z^{X_i^T}) ~ (1-p_{i0}) ~~~~ \text{since } (X_i | X_i>0) \text{ is zero-truncated r.v. } X_i^T \\
