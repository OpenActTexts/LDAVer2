...
*Chapter Preview.* This chapter introduces loss reserving (also known as claims reserving) for property and casualty (P&C, or general, non-life) insurance products. In particular, the chapter sketches some basic, though essential, analytic tools to assess the reserves on a portfolio of P&C insurance products. First, Section \@ref(S:Sec141) motivates the need for loss reserving, then Section \@ref(S:Sec142) studies the available data sources and introduces some formal notation to tackle loss reserving as a prediction challenge. Next, Section \@ref(S:Sec143) covers the chain-ladder method and Mack's distribution-free chain-ladder model. Section \@ref(S:Sec144) then develops a fully stochastic approach to determine the outstanding reserve with generalized linear models (GLMs), including the technique of bootstrapping to obtain a predictive distribution of the outstanding reserve via simulation. 

## Motivation {#S:Sec141}

Our starting point is the lifetime of a P&C insurance claim. Figure \@ref(fig:Fig141) pictures the development of such a claim over time and identifies the events of interest: 

(ref:Fig141)  **Lifetime or Run-off of a Claim**

```{tikz, Fig141, fig.cap = '(ref:Fig141)', fig.width=5, echo = FALSE}
...
(ref:Fig143)  **Lifetime of an RBNS Claim**

```{tikz, Fig143, fig.cap = '(ref:Fig143)', fig.width=5, echo = FALSE}
\usetikzlibrary{snakes}
...
(ref:Fig144)  **Lifetime of an IBNR Claim**

```{tikz, Fig144, fig.cap = '(ref:Fig144)', fig.width=5, echo = FALSE}
\usetikzlibrary{snakes}
\usetikzlibrary{shapes.arrows}
\definecolor{shadecolor}{HTML}{f67e8e}  %licht rood
\begin{tikzpicture}[scale=0.8,snake=zigzag, line before snake = 5mm, line after snake = 5mm]
...
(ref:Fig147) **A Run-off Triangle with Cumulative Payment Data.** *Source: @WuthrichMerz2008, Table 2.2.*

```{tikz Fig147, fig.cap = '(ref:Fig147)', echo = FALSE, fig.pos='h'}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
Whereas the triangle in Figure \@ref(fig:Fig146) displays incremental payment data, the  Figure \@ref(fig:Fig147) shows the same information in cumulative format. Now, cell $(2004,1)$ displays the total claim amount paid *up to* payment delay 1 for all claims that occurred in year 2004. Therefore, it is the sum of the amount paid in 2004 and the amount paid in 2005 on accidents that occurred in 2004. 

Different pieces of information can be stored in run-off triangles as those shown in  Figure \@ref(fig:Fig146) and  Figure \@ref(fig:Fig147). Depending on the kind of data stored, the triangle will be used to estimate different quantities. 
...
### Loss Reserve Notation {#S:Sec1423}

#### Run-off Triangles {-}

To formalize the displays shown in Figures \@ref(fig:Fig146) and \@ref(fig:Fig147), we let $i$ refer to the occurrence or accident year, the year in which the insured event happened. In our notation the first accident year considered in the portfolio is denoted with 1 and the latest, most recent accident year is denoted with $I$. Then, $j$ refers to the payment delay or development year, where a delay equal to 0 corresponds to the accident year itself.  Figure \@ref(fig:Fig148) shows a triangle where the same number of years is considered in both the vertical as well as the horizontal direction, hence $j$ runs from 0 up to $J = I-1$.

(ref:Fig148)  **Mathematical notation for a run-off triangle.** *Source:* @WuthrichMerz2008

```{tikz, Fig148, fig.cap = '(ref:Fig148)', echo = FALSE}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
the ultimate claim amount for occurrence year $i$, corresponding to the final development period $I-1$ in  Figure \@ref(fig:Fig147). We assume that - beyond this period - no further payments will follow, although this assumption can be relaxed.
...
### R Code to Summarize Loss Reserve Data {#S:Sec1424}
...
We display the triangle and recognize the numbers (in thousands) in Figure \@ref(fig:Fig147). Cells in the lower triangle are indicated as *not available*, `NA`.
...
## The Chain-Ladder Method {#S:Sec143}
...
### The Deterministic Chain-Ladder {#S:Sec1431}
...
$$ 
Thus, the development factor tells you how the cumulative amount in development year $j$ grows to the cumulative amount in year $j+1$. We highlight the cumulative amount in period 0 in blue and the cumulative amount in period 1 in red on the Figure \@ref(fig:Fig1410) taken from @WuthrichMerz2008 (Table 2.2, also used in @WuthrichMerz2015, Table 1.4).

(ref:Fig1410) **A Run-off Triangle with Cumulative Payment Data Highlighting the Cumulative Amount in Period 0 in Blue and the Cumulative Amount in Period 1 in Red.** *Source: @WuthrichMerz2008, Table 2.2.*

```{tikz, Fig1410, fig.cap = '(ref:Fig1410)',   fig.asp = 0.40, echo = FALSE}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
The chain-ladder method then presents an intuitive recipe to estimate or calculate these development factors. Since the first development factor $f_0$ describes the development of the cumulative claim amount from development period 0 to development period 1, it can be estimated as the ratio of the cumulative amounts in red and the cumulative amounts in blue, highlighted in the  Figure \@ref(fig:Fig1410). We then obtain the following estimate $\hat{f}_0^{CL}$ for the first development factor $f_0$, given observations $\mathcal{D}_I$:
$$
...
$$
Going forward with this reasoning, the next development factor $f_1$ can be estimated. Since $f_1$ captures the development from period 1 to period 2, it can be estimated as the ratio of the numbers in red and the numbers in blue as highlighted in Figure \@ref(fig:Fig1411).

(ref:Fig1411) **A Run-off Triangle with Cumulative Payment Data Highlighting the Cumulative Amount in Period 1 in Blue and the Cumulative Amount in Period 2 in Red.** *Source: @WuthrichMerz2008, Table 2.2.*

```{tikz, Fig1411, fig.cap = '(ref:Fig1411)',  echo = FALSE }
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
We continue analogously and obtain following predictions, printed in italics in the  Figure \@ref(fig:Fig1412).

(ref:Fig1412) **A Run-off Triangle with Cumulative Payment Data Including Predictions in Italic** *Source: @WuthrichMerz2008, Table 2.2.*

```{tikz, Fig1412, fig.cap = '(ref:Fig1412)',  echo = FALSE}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
Eventually we need to estimate the values in the final column. The last development factor $f_8$ measures the growth from development period 8 to development period 9 in the triangle. Since only the first row in the triangle has both cells observed, this last factor is estimated as the ratio of the value in red and the value in blue in Figure \@ref(fig:Fig1413). 

(ref:Fig1413) **A Run-off Triangle with Cumulative Payment Data Highlighting the Cumulative Amount in Period 8 in Blue and the Cumulative Amount in Period 9 in Red.** *Source: @WuthrichMerz2008, Table 2.2.*

```{tikz, Fig1413, fig.cap = '(ref:Fig1413)', echo = FALSE}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
This yields the following  Figure \@ref(fig:Fig1414):

(ref:Fig1414) **A Run-off Triangle with Cumulative Payment Data Including Predictions in Italic** *Source: @WuthrichMerz2008, Table 2.2.*

```{tikz, Fig1414, fig.cap = '(ref:Fig1414)', echo = FALSE}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
\begin{tabular}{|c||r r r r r r r r r r|}
\hline
...
$$
This is the chain-ladder estimate for the reserve necessary to fulfill future liabilities with respect to claims that occurred in this particular occurrence period. These reserves per occurrence period and for the total summed over all occurrence periods are summarized in Figure \@ref(fig:Fig1415). 

(ref:Fig1415) **Reserves per Occurrence Period and for Total**

```{tikz, Fig1415, fig.cap = '(ref:Fig1415)', echo = FALSE}
\begin{tikzpicture}[scale=0.3] 
\node at (0,0) {
...
### Mack's Distribution-Free Chain-Ladder Model {#S:Sec1432}
...
In this section we will focus on the distribution-free chain-ladder model as an underlying stochastic model, introduced in @Mack1993. This method allows us to estimate the standard errors of the chain-ladder predictions. In the next Section \@ref(S:Sec144), generalized linear models are used to develop a fully stochastic approach for predicting the outstanding reserve.
...
## GLMs and Bootstrap for Loss Reserves {#S:Sec144}
...
## Further Resources and Contributors {#S:Sec145}
...
As displayed in Figure \@ref(fig:Fig141), similar timelines and visualizations are discussed (among others) in @WuthrichMerz2008, @AntonioPlat2014 and @WuthrichMerz2015. 
